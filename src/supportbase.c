...
static int queryISOGameListCache(const struct game_cache_list *cache, base_game_info_t *ginfo, const char *filename)
{
    ...
    for (i = 0; i < cache->count; i++) {
        snprintf(isoname, sizeof(isoname), "%s%s", cache->games[i].indexName, cache->games[i].extension);
...
static int scanForISO(char *path, char type, struct game_list_t **glist)
{
    ...
    if ((temp = fileXioGetStat(txtPath, &fileStat)) >= 0) {
        ...
        if (strcmp(curModiTime, cache.games[0].preModiTime) == 0) {
            ...
            fprintf(debugFile, "文件时间为：%02u%02u%02u%02u\r\n", fileStat.mtime[0], fileStat.mtime[1], fileStat.mtime[2], fileStat.mtime[3]);
            fprintf(debugFile, "缓存时间为：%02u%02u%02u%02u\r\n", (cache.games[0].preModiTime)[0], (cache.games[0].preModiTime)[1], (cache.games[0].preModiTime)[2], (cache.games[0].preModiTime)[3]);
        }
    }
