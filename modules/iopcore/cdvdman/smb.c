...
/* Limit the maximum chunk size of receiving operations, to avoid triggering the congestion avoidance algorithm of the SMB server.
   ...
   该值应小于TCP窗口，当前游戏内TCP窗口为40960字节。 */
#define CLIENT_MAX_BUFFER_SIZE 32768     // 最多允许接收32768字节。
...
int smb_ReadFile(u16 FID, u32 offsetlow, u32 offsethigh, void *readbuf, int nbytes)
{
    ...
    while (remaining > 0) {
        toRead = remaining >= CLIENT_MAX_RECV_SIZE ? CLIENT_MAX_RECV_SIZE : (remaining >= 16384 ? 16384 : (remaining > 8192 ? 8192 : remaining));
